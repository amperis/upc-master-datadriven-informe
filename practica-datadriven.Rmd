...
author: 'Grupo G: Yaneth Gonzalez, Miriam JimÃ©nez, Anabel Vilchez, Alejandro Moreno'
date: "`r Sys.Date()`"
output:
  html_document:
    theme: united
...
El propósito de la práctica es evidenciar las posibles vulnerabilidades industriales que pueden darse en diferenres paises. La pregunta que intentamos responder es si **existe algun pais más propenso a tener vulnerabilidades industriales**.

Para ello buscaremos en [Shodan](http://www.shodan.io) todos los dispositivos industriales pÃºblicos en Internet y lo cruzaremos con las vulnerabilidades que tengan este tipo de modelos de dispositivos industriales publicos en la [Common Vulnerabilities and Exposures](https://cve.mitre.org). El resultado lo representaremos en un mapa y realizaremos un analisis.

Shodan es un motor de bÃºsqueda que permite al usuario encontrar iguales o diferentes tipos especÃ­ficos de equipos (routers, servidores, etc.) conectados a Internet a travÃ©s de una variedad de filtros
...
Esta guÃ­a puede encontratse en:

- https://github.com/amperis/upc-master-datadriven-informe


# 2. DescripciÃ³n de los modelos de datos

Las fuentes de datos que utilizaremos en la prÃ¡ctica serÃ¡n las siguientes:

- Fuente de datos con dispositivos industriales segÃºn fabricantes
- Fuente de datos Shodan de dispositivos industriales accesibles desde Internet
- Fuente de datos de CVE 

A continuaciÃ³n se describen las caracteristicas de cada una de las fuentes de datos.

## 2.1 Fuente de datos con dispositivos industriales segÃºn fabricantes

Esta fuente de datos es un archivo CSV generado manualmente a travÃ©s de los diferentes dispositivos industriales mÃ¡s comunes actualmente. Nos hemos basado en diferente documentacion tÃ©cnica disponible en Internet asÃ­ como en la docuemntacion impartida en el master. Un ejemplo de esta informaciÃ³n se puede enconrar en la siguiente URL:

- https://cybersecuritylaboratory.wordpress.com/2016/10/05/utilizando-shodan-para-encontrar-sistemas-de-control-industrial

Esta lista debe actualizarse a medida que se conozcan mÃ¡s fabricantes. Las cabeceras de dicha tabla son las siguientes:
...
A continuaciÃ³n se muestran los 15 primeros regisotros de este CSV:
...
Esta fuente de datos es un archivo de datos CVE obtenida a travÃ©s de un proceso interativo programado en R desde el cual por cada dispositivo industrial obtenido del punto anterior es buscado en Shodan. Esta busqueda me devuelve diferentes informaciÃ³n del dispositivo industrial encontrado y publico en Internet.
...
aux.matches.location.city | string de la ciudad donde estÃ¡ ubicada fisicamente dicha IP
...
A continuaciÃ³n se muestran los 10 primeros regisotros de este CSV:
...
A continuaciÃ³n se muestran los 3 primeros registros de CVE:
...
# 3. Objetivos del anÃ¡lisis de datos

# 4. AnÃ¡lisis de datos

A continuaciÃ³n se analizan los paises que mÃ¡s dispositivos industriales publicos en Internet se han encontrado a travÃ©s de Shodan. Como puede observarse el pais que mÃ¡s dispositivos tiene con gran diferencia es EEUU:
...
A continuaciÃ³n se analizan cuales son los 10 fabricantes que mÃ¡s vulnerabilidades. Para ello se han analizado todos los dispositivos Shodan encontrados y se han acumulados sus vulnerabilidades. Como conclusiÃ³n podemos afirmar que el fabricante que mÃ¡s dispositivos tiene con vulveravilidades publicas en CVE es Elster Energy.
...
A continuaciÃ³n se analizan cuales son los paises que mÃ¡s vulnerablidades tienen:
